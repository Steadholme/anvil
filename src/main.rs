//! Anvil entry point: init state from env (select store, migrate, spawn the run scheduler), serve.
//!
//! Also exposes a dependency-free `anvil healthcheck` subcommand used as the container
...
async fn main() {
    ...
    let state = match anvil::build_state_from_env().await {
        Ok(state) => state,
        Err(e) => {
    ...
    let app = anvil::app(state);

    let listener = tokio::net::TcpListener::bind(addr)
        .await
        .unwrap_or_else(|e| panic!("failed to bind {addr}: {e}"));

    tracing::info!(%addr, "Anvil listening (CI build runner)");
    axum::serve(listener, app).await.expect("server error");
}

/// GET `/healthz` over a raw TCP socket. Returns process exit code (0 = healthy).
fn run_healthcheck() -> i32 {
    let bind_addr = std::env::var("BIND_ADDR").unwrap_or_else(|_| "127.0.0.1:9240".to_string());
    // Healthcheck always talks to the loopback regardless of the bind interface.
    let port = bind_addr.rsplit(':').next().unwrap_or("9240");
